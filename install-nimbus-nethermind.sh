...
linux_install_validator-install() {
    ...
    git clone https://github.com/OpenFusionist/EthPillar-Endurance.git ~/git/ethpillar 2> /dev/null || (cd ~/git/ethpillar ; git fetch origin feat/devnet ; git checkout feat/devnet ; git pull)
