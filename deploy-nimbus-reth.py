...
def download_and_install_reth():
    if execution_client == 'reth':
        ...
        ##### RETH SERVICE FILE ###########
        reth_exec_flag = f'''node --full --log.file.directory=/var/lib/reth/log --datadir=/var/lib/reth --port={EL_P2P_PORT} --http --http.port={EL_RPC_PORT} --authrpc.jwtsecret={JWTSECRET_PATH} --metrics=0.0.0.0:6060 --authrpc.addr=0.0.0.0 --authrpc.port=8551'''
